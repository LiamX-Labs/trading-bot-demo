...
def fetch_symbols():
    """OPTIMIZED: Fetch only top 40 highest volume symbols"""
    resp = requests.get(f"{BASE_URL}/v5/market/tickers?category=linear").json()
    syms = []
    for it in resp.get("result", {}).get("list", []):
        # CHANGED: Increased volume threshold to 20M
        if float(it.get("turnover24h",0)) > 20_000_000 and it["symbol"].endswith("USDT"):
            syms.append(it["symbol"])
    
    # ADDED: Sort by volume and limit to top 40
...
async def fetch_historical_klines_async(session, symbol, interval="30", limit=100):
...
async def load_all_historical_data(symbols):
    ...
    # CHANGED: Reduced connection limits for t2.micro
    connector = aiohttp.TCPConnector(limit=10, limit_per_host=5)
    timeout = aiohttp.ClientTimeout(total=5)
    
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        # CHANGED: Reduced concurrent requests from 10 to 3
        semaphore = asyncio.Semaphore(3)
        
        async def fetch_with_semaphore(symbol):
            async with semaphore:
                data = await fetch_historical_klines_async(session, symbol, "30", 100)
                return symbol, data
...
def apply_indicators(df):
    ...
    df["volatility"] = returns.rolling(24).std()
    df["spread"] = (df["high"] - df["low"]) / df["close"] * 100
    
    price_change = df["close"].pct_change(24) * 100
    volume_change = df["volume"].pct_change(24) * 100
...
async def process_kline(msg):
    ...
    try:
        ...
        if not topic.startswith("kline.30.") or not isinstance(data, list):
            return
...
async def balance_monitor():
    ...
    global last_update_ts
    while True:
        await asyncio.sleep(300)  # CHANGED: 60s -> 300s (5 min)
        try:
...
async def pnl_fallback_monitor():
    global last_update_ts
    while True:
        await asyncio.sleep(10)  # CHANGED: 5s -> 10s
        try:
            await risk_mgr.check_unrealized_drawdown()
            last_update_ts = time.time()
        except Exception as e:
    ...
    while True:
        ...
        for symbol in history:
            if len(history[symbol]) > 50:
                history[symbol] = deque(list(history[symbol])[-50:], maxlen=50)
...
async def main():
    try:
        ...
        for symbol, data in historical_data.items():
            history[symbol] = deque(maxlen=50)  # CHANGED: 100 -> 50
            if data:
        ...
        # ENHANCED: Start monitors with reconciliation
        monitor_tasks = [
